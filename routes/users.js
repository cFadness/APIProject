...
router.post('/:userId/shoppingcart/:productId', async (req, res) => {
    try {
        ...
        if (!user) return res.status(400).send(`The user with id "${req.params.userId}"
        does not exist.`);

        const product = await Product.findById(req.params.productId);
        if (!product) return res.status(400).send(`The product with id "${req.params.productID}"
        does not exist.`);
...
router.put('/:userId/shoppingcart/:productId', async (req, res) => {
    try {
        ...
        if (!product) return res.status(400).send(`The product with id "${req.params.productId}"
        does not exist in the user's shopping cart.`);
